// ============================================================
// app.js — Client polling 2s, diff DOM, rôles spéciaux
// ============================================================
...
function getOrCreateId() {
  ...
  if (!id) { id = 'p_' + Math.random().toString(36).substring(2, 12); localStorage.setItem('_pid', id); }
  return id;
}
...
// editRoles = alias direct sur config.customRoles
...
// ─── Helpers ──────────────────────────────────────────────────
function impRolesCount()  { return S.config.customRoles.filter(r => r.type === 'impostor').length; }
function crewRolesCount() { return S.config.customRoles.filter(r => r.type === 'crewmate').length; }

function clampSpecialCounts() {
  S.config.specialImpCount  = Math.min(S.config.specialImpCount  || 0, impRolesCount());
  S.config.specialCrewCount = Math.min(S.config.specialCrewCount || 0, crewRolesCount());
}

function updateSpecialSteppers() {
  const sImpEl  = document.getElementById('srole-imp-val');
  const sCrewEl = document.getElementById('srole-crew-val');
  if (sImpEl)  sImpEl.textContent  = String(S.config.specialImpCount  || 0);
  if (sCrewEl) sCrewEl.textContent = String(S.config.specialCrewCount || 0);
}
...
function applyRoomState(room) {
  ...
  // Si un input de rôle a le focus → on garde la config locale (pas d'écrasement pendant la frappe)
  const editing = document.getElementById('roles-list')?.contains(document.activeElement);
  if (!editing) {
    // Applique la config serveur en conservant les specialCounts si le serveur ne les a pas
    S.config = {
      ...room.config,
      specialImpCount:  room.config.specialImpCount  ?? S.config.specialImpCount  ?? 0,
      specialCrewCount: room.config.specialCrewCount ?? S.config.specialCrewCount ?? 0,
    };
  }

  const me = room.players.find(p => p.id === S.playerId);
  if (me?.role && !S.myRole) {
    S.myRole = me.role; S.myCustomRole = me.customRole; S.myDesc = me.description;
  ...
  if (prevPhase !== room.phase) handlePhaseChange(room.phase);

  if (room.phase === 'lobby') {
    renderLobby();
  } else if (room.phase === 'discussion' || room.phase === 'vote') {
    renderGamePlayersIfChanged();
    if (S._prevPhase !== room.phase) { updatePhaseBadge(room.phase); S._prevPhase = room.phase; }
...
function handlePhaseChange(newPhase) {
  if (newPhase === 'discussion') {
    showScreen('game');
    S.hasVoted = false; S._prevPlayersHash = null;
    updatePhaseBadge('discussion'); clearChat(); S.lastMsgCount = 0; updateMyRoleCard();
  } else if (newPhase === 'vote') {
    S.hasVoted = false; S._prevPlayersHash = null; updatePhaseBadge('vote');
...
function syncTimer(timerEnd, phase) {
  ...
  if (Math.abs(S.timerValue - remaining) > 3 || S.clientTimer === null) startClientTimer(remaining, max);
}
function startClientTimer(seconds, max) {
  stopClientTimer();
  S.timerMax = max || seconds; S.timerValue = seconds;
...
function renderLobby() {
  ...
  if (countEl.textContent !== String(S.players.length)) countEl.textContent = String(S.players.length);
  ...
  S.players.forEach(p => {
    ...
    const cls  = `player-card${p.isHost ? ' is-host' : ''}`;
    const html = `
    ...
    if (!el) {
      ...
      el.dataset.pid = p.id; el.className = cls; el.innerHTML = html;
      grid.appendChild(el);
    } else {
      if (el.className !== cls) el.className = cls;
      if (el.innerHTML.replace(/\s+/g,' ').trim() !== html.replace(/\s+/g,' ').trim()) el.innerHTML = html;
    }
  });
}
...
function renderRolesList() {
  ...
  // Ne reconstruit pas si un input a le focus → évite le saut curseur
    ...
    el.innerHTML = `
      ...
      <div class="role-item-top">
        ...
        <div class="role-toggle">
          <button class="rtbtn ${r.type === 'crewmate' ? 'ac' : ''}" onclick="setRoleType(${i},'crewmate')">Équipier</button>
          <button class="rtbtn ${r.type === 'impostor' ? 'ai' : ''}" onclick="setRoleType(${i},'impostor')">Imposteur</button>
        </div>
      </div>
...
function renderGamePlayersIfChanged() {
  const h = hashPlayers(S.players) + ':' + S.phase + ':' + S.hasVoted;
  if (h === S._prevPlayersHash) return;
  S._prevPlayersHash = h;
  renderGamePlayers();
}
...
function renderGamePlayers() {
  ...
  S.players.forEach(p => {
    ...
    const classes = ['gp-item', !p.isAlive?'dead':'', canVote?'vote-target':'', p.hasVoted&&isVote?'voted':''].filter(Boolean).join(' ');
    const html = `
    ...
    if (!el) {
      el = document.createElement('div');
      el.dataset.pid = p.id; el.className = classes; el.innerHTML = html;
      list.appendChild(el);
    } else {
...
async function doVote(targetId) {
  ...
  } catch(e) { S.hasVoted = false; showToast(e.message); }
}
...
function initRoom(data) {
  ...
  S.config    = {
    ...data.room.config,
    specialImpCount:  data.room.config.specialImpCount  ?? 0,
    specialCrewCount: data.room.config.specialCrewCount ?? 0,
  };
...
async function startGame() {
  ...
  S.hasVoted = false; S.lastMsgCount = 0; S._prevPlayersHash = null;
  try {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('avatar-close').addEventListener('click',  () => document.getElementById('avatar-modal').classList.add('hidden'));
  ...
  // Stepper rôles spéciaux imposteurs (min 0, max = nb rôles imp définis)
  ...
  document.getElementById('srole-imp-plus').addEventListener('click', () => {
    const cur = S.config.specialImpCount || 0;
    const max = impRolesCount();
    if (max === 0) return showToast('Ajoute d\'abord des rôles imposteurs !');
    if (cur >= max) return showToast(`Maximum : ${max} rôle(s) imposteur défini(s)`);
    S.config.specialImpCount = cur + 1;
    document.getElementById('srole-imp-val').textContent = S.config.specialImpCount;
    saveConfig();
  });

  // Stepper rôles spéciaux équipiers (min 0, max = nb rôles crew définis)
  ...
  document.getElementById('srole-crew-plus').addEventListener('click', () => {
    const cur = S.config.specialCrewCount || 0;
    const max = crewRolesCount();
    if (max === 0) return showToast('Ajoute d\'abord des rôles équipiers !');
    if (cur >= max) return showToast(`Maximum : ${max} rôle(s) équipier défini(s)`);
    S.config.specialCrewCount = cur + 1;
    document.getElementById('srole-crew-val').textContent = S.config.specialCrewCount;
    saveConfig();
  });
  ...
  document.getElementById('btn-close-role').addEventListener('click', () => {
    ...
    showScreen('game'); updateMyRoleCard(); renderGamePlayers(); updatePhaseBadge(S.phase);
  });
  ...
  document.getElementById('btn-restart').addEventListener('click', () => {
    showScreen('lobby'); S.phase = 'lobby'; startPolling(); startGame();
  });
  document.getElementById('btn-home').addEventListener('click', () => location.reload());

  window.addEventListener('beforeunload', () => {
    if (S.roomCode) navigator.sendBeacon('/api/room?action=leave',
